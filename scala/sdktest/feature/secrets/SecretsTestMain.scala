...
import voxgig.bluefinshieldconexsdk.core._
import voxgig.bluefinshieldconexsdk.feature._
...
object SecretsTestMain {
  ...
  private val ENVPREFIX = "BLUEFIN_SHIELDCONEX_TEST_SECRETS_"
  ...
  private def secretsFeatureOf(client: BluefinShieldconexSDK): SecretsFeature = {
  ...
  private def withSecrets(build: Boolean => BluefinShieldconexSDK): BluefinShieldconexSDK = {
    val client = build(false)
    if (null == secretsFeatureOf(client)) build(true) else client
  }

  private def secretsClient(w: Wire, extra: (String, Object)*): BluefinShieldconexSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      new BluefinShieldconexSDK(opts)
    }

  private def testModeClient(w: Wire, extra: (String, Object)*): BluefinShieldconexSDK =
    withSecrets { extend =>
      val opts = sdkopts(w, extra*)
      if (extend) opts.put("extend", jlist(new SecretsFeature()))
      BluefinShieldconexSDK.testSDK(null, opts)
    }
  ...
  private def entities(client: BluefinShieldconexSDK): List[SdkEntity] = {
  ...
  private def driveUntil(client: BluefinShieldconexSDK, stop: () => Boolean): (Boolean, String) = {
    var lasterr = ""
    var done = false
  ...
  private def driveOneCall(client: BluefinShieldconexSDK, w: Wire): Boolean = {
  ...
  private def testInactive(): Unit = {
    val client = BluefinShieldconexSDK.testSDK(null, jmap("apikey" -> "OPTKEY01"))
    ...
    val bare = BluefinShieldconexSDK.testSDK(null, null)
