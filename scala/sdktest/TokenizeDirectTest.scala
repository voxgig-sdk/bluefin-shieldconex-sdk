// Generated direct-call tests for the tokenize entity (mirrors the java
...
object TokenizeDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-list-tokenize") {
      ...
      rep.eq("direct-list-tokenize.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-list-tokenize.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-list-tokenize.islist", result.get("data").isInstanceOf[JList[?]], "expected data to be an array, got " + result.get("data"))
      val listData = result.get("data").asInstanceOf[JList[Object]]
      rep.eqI("direct-list-tokenize.size", 2, listData.size())
      rep.eqI("direct-list-tokenize.calls", 1, setup.calls.size())
    }
