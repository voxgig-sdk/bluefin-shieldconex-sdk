...
import voxgig.bluefinshieldconexsdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefinshieldconexsdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinShieldconexSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinShieldconexSDK)(f: (Context, Seq[Value]) => Value): Unit =
  ...
  private def clientFor(name: String, shared: BluefinShieldconexSDK): BluefinShieldconexSDK =
    sectionSetup(name) match {
      case m @ VMap(_) => BluefinShieldconexSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinShieldconexSDK.testSDK()
