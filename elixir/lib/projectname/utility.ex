# BluefinShieldconex SDK utility
#
...
defmodule BluefinShieldconex.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinShieldconex.Helpers, as: H
  alias BluefinShieldconex.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinShieldconexSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinShieldconex.Config.shared_config) and merge uses its nested nodes as
      ...
      resolved =
        Regex.replace(~r/\{([A-Za-z0-9_]+)\}/, base, fn _match, name ->
          ...
          cond do
            ...
            true ->
              raise BluefinShieldconex.Error,
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinShieldconex.Error{}, err) -> err.msg
    ...
    msg = "BluefinShieldconexSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = %BluefinShieldconex.Error{
      code: "",
      msg: msg,
      sdk: "BluefinShieldconex",
      ctx: ctx,
    ...
    sdk_err = if match?(%BluefinShieldconex.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinShieldconex.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinShieldconex.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
