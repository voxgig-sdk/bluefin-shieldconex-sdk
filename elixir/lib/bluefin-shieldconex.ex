...
defmodule BluefinShieldconex do
  alias Voxgig.Struct, as: S
  alias BluefinShieldconex.Helpers, as: H
  alias BluefinShieldconex.{Utility, Context, Spec}
  ...
  def new(options \\ nil) do
    ...
    config = BluefinShieldconex.Config.shared_config()
    ...
    if S.islist(feature_order) and S.size(feature_order) > 0 do
      Enum.each(0..(S.size(feature_order) - 1), fn i ->
        ...
        if fom != nil and S.getprop(fom, "active") == true do
          Utility.feature_add(rootctx, BluefinShieldconex.Features.make_feature(fname))
        end
      end)
    end
      ...
      _ ->
        ...
        cond do
          ...
          S.ismap(fetched) ->
            ...
            content_length =
              if S.ismap(headers), do: BluefinShieldconex.Feature.header_get(headers, "content-length"), else: nil
  ...
  def graphql(client, query, variables \\ nil, ctrl \\ nil) do
    if not op_allowed?(client, "graphql") do
      ...
      if count == 0 do
        ...
        S.setprop(res, "err", "BluefinShieldconexSDK: graphql: " <> msg)
  ...
  @doc "Entity factory for detokenize."
  def detokenize(client, entopts \\ nil) do
    BluefinShieldconex.Entity.Detokenize.new(client, entopts)
  end

  @doc "Entity factory for tokenize."
  def tokenize(client, entopts \\ nil) do
    BluefinShieldconex.Entity.Tokenize.new(client, entopts)
  end

  @doc "Entity factory for tokenize_batch."
  def tokenize_batch(client, entopts \\ nil) do
    BluefinShieldconex.Entity.TokenizeBatch.new(client, entopts)
  end

  @doc "Entity factory for tokenize_read."
  def tokenize_read(client, entopts \\ nil) do
    BluefinShieldconex.Entity.TokenizeRead.new(client, entopts)
  end

  @doc "Entity factory for validate."
  def validate(client, entopts \\ nil) do
    BluefinShieldconex.Entity.Validate.new(client, entopts)
  end
