...
defmodule BluefinShieldconex.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinShieldconex.Omni, as: O
  alias BluefinShieldconex.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinShieldconex.get_utility(sdk)
    rootctx = BluefinShieldconex.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinShieldconex.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinShieldconex.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinShieldconex.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinShieldconex.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinShieldconex.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinShieldconex.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinShieldconex.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinShieldconex.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinShieldconex.test(nil, setup_opts(spec, "prepareAuth"))
