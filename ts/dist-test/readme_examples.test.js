...
const __1 = require("..");
const SDK_NAME = 'BluefinShieldconexSDK';
...
const TEST_SEED = { "entity": { "detokenize": { "test01": { "id": "test01" } }, "tokenize": { "test01": { "id": "test01" } }, "tokenize_batch": { "test01": { "id": "test01" } }, "tokenize_read": { "test01": { "id": "test01" } }, "validate": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BluefinShieldconexSDK\b/.test(code) ||
        /\bBluefinShieldconexSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BluefinShieldconexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BluefinShieldconexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
