# BluefinShieldconex TypeScript SDK



The TypeScript SDK for the BluefinShieldconex API — a type-safe, entity-oriented client with full async/await support.
...
const client = new BluefinShieldconexSDK({
  ...
  secret: process.env.BLUEFIN_SHIELDCONEX_SECRET,
})
```

### 2. List detokenize records

`list()` resolves to an array of Detokenize ENTITIES — every operation
resolves to entities, not raw records. Iterate them directly, and call
`.data()` on one for the record it holds:

```ts
const detokenizes = await client.Detokenize().list({ bfid: "example" })
...
```ts
// Create — returns the created Detokenize ENTITY (.data() for the record)
...
```ts
const client = new BluefinShieldconexSDK({ apikey: '...', secret: '...' })
...
BLUEFIN_SHIELDCONEX_SECRET=<your-secret>
```
...
```ts
const tokenizes = await client.Tokenize().list({ template_ref: "example" })
```
...
| `messageId` | `string` | Message Id |
| `reference` | `string` | Request Reference. |
| `state` | `Record<string, any>` | Tokenized State Data (if available) |
| `values` | `any[]` |  |
...
bluefin-shieldconex/
├── src/
│   ├── BluefinShieldconexSDK.ts        # Main SDK class
