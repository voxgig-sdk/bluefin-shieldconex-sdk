...
import 'entity/DetokenizeEntity.dart';
import 'entity/TokenizeEntity.dart';
import 'entity/TokenizeBatchEntity.dart';
import 'entity/TokenizeReadEntity.dart';
import 'entity/ValidateEntity.dart';


export 'BluefinShieldconexTypes.dart';
export 'entity/DetokenizeEntity.dart';
export 'entity/TokenizeEntity.dart';
export 'entity/TokenizeBatchEntity.dart';
export 'entity/TokenizeReadEntity.dart';
export 'entity/ValidateEntity.dart';
...
import 'Config.dart';
import 'Spec.dart';
// BluefinShieldconexEntityBase / BluefinShieldconexError / BaseFeature are re-exported below;
...
// `utility` (BluefinShieldconexTypes.dart) — exporting both from this library makes
// the name ambiguous and NO dart SDK for such an API compiles:
//
//   Error: 'Utility' is exported from both 'lib/BluefinShieldconexTypes.dart'
...
export 'BluefinShieldconexEntityBase.dart' show BluefinShieldconexEntityBase;
export 'BluefinShieldconexError.dart' show BluefinShieldconexError;
...
class BluefinShieldconexSDK {
  ...
  BluefinShieldconexSDK([dynamic options]) {
  ...
  dynamic _opDenied(String op) {
    ...
    return {
      'ok': false,
      'err': Exception('BluefinShieldconexSDK: $op: operation not allowed by'
  ...
  Future<dynamic> graphql(String query,
    ...
    if (errors is List && 0 < errors.length) {
      ...
      res['ok'] = false;
      res['err'] = Exception('BluefinShieldconexSDK: graphql: '
  ...
  DetokenizeEntity Detokenize([dynamic entopts]) {
    return DetokenizeEntity(this, entopts);
  }


  // Entity access: `client.Tokenize().list()` / `client.Tokenize().load({'id': ...})`.
  TokenizeEntity Tokenize([dynamic entopts]) {
    return TokenizeEntity(this, entopts);
  }


  // Entity access: `client.TokenizeBatch().list()` / `client.TokenizeBatch().load({'id': ...})`.
  TokenizeBatchEntity TokenizeBatch([dynamic entopts]) {
    return TokenizeBatchEntity(this, entopts);
  }


  // Entity access: `client.TokenizeRead().list()` / `client.TokenizeRead().load({'id': ...})`.
  TokenizeReadEntity TokenizeRead([dynamic entopts]) {
    return TokenizeReadEntity(this, entopts);
  }


  // Entity access: `client.Validate().list()` / `client.Validate().load({'id': ...})`.
  ValidateEntity Validate([dynamic entopts]) {
    return ValidateEntity(this, entopts);
  }



  static BluefinShieldconexSDK test([dynamic testoptsarg, dynamic sdkoptsarg]) {
    ...
    final testsdk = BluefinShieldconexSDK(sdkopts);
    testsdk.mode = 'test';

    return testsdk;
  }

  BluefinShieldconexSDK tester([dynamic testopts, dynamic sdkopts]) {
  ...
  @override
  String toString() {
    return 'BluefinShieldconex ' + _utility.struct.jsonify(toJSON());
  }
}

typedef SDK = BluefinShieldconexSDK;
