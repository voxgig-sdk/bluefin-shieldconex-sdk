...
void tests() {
  describe('TokenizeBatchEntity', () {
    ...
    test('stream', (t) async {
      ...
      final testsdk = BluefinShieldconexSDK.test(seed, sdkopts);
      final ent = testsdk.TokenizeBatch();
      ...
      final plainsdk = BluefinShieldconexSDK.test(seed);
      final plainent = plainsdk.TokenizeBatch();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'tokenize_batch.' + op, live)) {
          return;
        }
      }
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['tokenize_batch01', 'tokenize_batch02', 'tokenize_batch03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_SHIELDCONEX_TEST_TOKENIZE_BATCH_ENTID'];
