// BluefinShieldconex SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinShieldconexSDK> plClient(const Value& sdkopts) {
  return BluefinShieldconexSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinShieldconexSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinShieldconexSDK> client) {
  std::string out;
