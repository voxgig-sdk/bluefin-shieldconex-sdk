...
inline Value env_override(Value m) {
  std::string live = getenv_local("BLUEFINSHIELDCONEX_TEST_LIVE");
  std::string over = getenv_local("BLUEFINSHIELDCONEX_TEST_OVERRIDE");
  ...
  std::string explain = getenv_local("BLUEFINSHIELDCONEX_TEST_EXPLAIN");
  if (!explain.empty()) map_put(m, "BLUEFINSHIELDCONEX_TEST_EXPLAIN", Value(explain));
  return m;
}
