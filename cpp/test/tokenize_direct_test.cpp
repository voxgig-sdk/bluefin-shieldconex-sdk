// Generated direct-call tests for the tokenize entity (unit mode;
...
static TokenizeDirectSetup tokenize_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void tokenize_direct_list() {
  auto setup = tokenize_direct_setup(vlist({
    vmap({{"id", Value("direct01")}}),
    vmap({{"id", Value("direct02")}})
  }));
  auto sk = is_control_skipped("direct", "direct-list-tokenize", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("healthcheck/tokenize")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(tokenize_direct_list);
  return sdktest::summary("tokenize_direct_test");
}
