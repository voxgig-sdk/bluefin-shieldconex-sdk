...
//    only - a typed BluefinShieldconexContext there would make every ctx
...
final class BluefinShieldconexOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BluefinShieldconexContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BluefinShieldconexSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BluefinShieldconexResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BluefinShieldconexError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BluefinShieldconexResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BluefinShieldconexError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BluefinShieldconexError(is_string($code) ? $code : '', $msg);
    }
