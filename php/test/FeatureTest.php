...
// BluefinShieldconex SDK feature test
//
...
class FtCtrl extends BluefinShieldconexControl
{
...
class FtClient
{
    ...
    private BluefinShieldconexUtility $utility;

    public function __construct(BluefinShieldconexUtility $utility, array $options)
    {
    ...
    public function get_utility(): BluefinShieldconexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BluefinShieldconexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BluefinShieldconexContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BluefinShieldconexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BluefinShieldconexConfig::make_feature($name);
    ...
    private static function buildUrl(BluefinShieldconexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BluefinShieldconexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BluefinShieldconexResponse($fetched);
        }
    ...
    private function fail(BluefinShieldconexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BluefinShieldconexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
