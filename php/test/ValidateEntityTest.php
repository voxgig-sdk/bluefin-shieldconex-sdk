...
class ValidateEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_SHIELDCONEX_TEST_VALIDATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $validate_ref01_data = Helpers::to_map(is_object($validate_ref01_data_result) && method_exists($validate_ref01_data_result, 'data_get') ? $validate_ref01_data_result->data_get() : $validate_ref01_data_result);
...
function validate_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_SHIELDCONEX_TEST_VALIDATE_ENTID");
