...
// BluefinShieldconex SDK Detokenize entity
...
class DetokenizeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DetokenizeEntity($this->_client, $opts);
    }

    /**
     * @param Detokenize|array $args Detokenize data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BluefinShieldconexHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Detokenize|array The current Detokenize data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Detokenize fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BluefinShieldconexHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Detokenize fields).
     */
    ...
    /**
     * List Detokenize items matching the given filter.
     *
     * @param DetokenizeListMatch|array|null $reqmatch Match filter (any subset
     *   of Detokenize fields) as an assoc-array; DetokenizeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Detokenize[]|array A list of Detokenize items as assoc-arrays at
     *   the SDK boundary; throws BluefinShieldconexError on failure (item-5 convention).
     */
    ...
    /**
     * Create a new Detokenize.
     *
     * @param DetokenizeCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed DetokenizeCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Detokenize|array The created Detokenize as an assoc-array at the
     *   SDK boundary; throws BluefinShieldconexError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = BluefinShieldconexHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
