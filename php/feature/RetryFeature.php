...
// BluefinShieldconex SDK retry feature
...
class BluefinShieldconexRetryFeature extends BluefinShieldconexBaseFeature
{
    ...
    public function init(BluefinShieldconexContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BluefinShieldconexContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BluefinShieldconexContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BluefinShieldconexContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
