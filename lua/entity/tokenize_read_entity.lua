...
local TokenizeReadEntity = {}
TokenizeReadEntity.__index = TokenizeReadEntity


function TokenizeReadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TokenizeReadEntity)
  self._name = "tokenize_read"
  self._client = client
...
function TokenizeReadEntity:get_name()
  return self._name
end


function TokenizeReadEntity:make()
  local opts = {}
...
function TokenizeReadEntity:mark_deleted()
  self._deleted = true
end


function TokenizeReadEntity:deleted()
  return true == self._deleted
end


function TokenizeReadEntity:data_set(args)
  if args ~= nil then
...
function TokenizeReadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TokenizeReadEntity:match_set(args)
  if args ~= nil then
...
function TokenizeReadEntity:match_get()
...
function TokenizeReadEntity:stream(action, args, callopts)
...
---@param reqdata TokenizeReadCreateData
---@param ctrl? table
---@return TokenizeRead
---@return string? err
function TokenizeReadEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function TokenizeReadEntity:_run_op(ctx, post_done)
  local utility = self._utility
