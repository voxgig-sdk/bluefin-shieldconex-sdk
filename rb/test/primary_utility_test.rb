...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = BluefinShieldconexBaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = BluefinShieldconexSDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = BluefinShieldconexSDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = BluefinShieldconexSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = BluefinShieldconexSDK.new({
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
        },
  ...
  def test_make_spec_basic
    ...
    spec_client = BluefinShieldconexSDK.test(nil, setup_opts)
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = BluefinShieldconexOperation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = BluefinShieldconexSDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = BluefinShieldconexContext.new(ctxmap, nil)
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    BluefinShieldconexError.new(code, msg)
  end
end
