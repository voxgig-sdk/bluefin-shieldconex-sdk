...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = BluefinShieldconexOmni.make_runner(TEST_JSON_FILE, BluefinShieldconexSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    BluefinShieldconexError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : BluefinShieldconexOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = BluefinShieldconexOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
