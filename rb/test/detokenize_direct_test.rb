...
class DetokenizeDirectTest < Minitest::Test
  def test_direct_list_detokenize
    setup = detokenize_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-detokenize", setup[:live] ? "live" : "unit")
    if _should_skip
...
def detokenize_direct_setup(mockres)
