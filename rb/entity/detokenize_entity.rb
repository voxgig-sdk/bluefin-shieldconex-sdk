...
class DetokenizeEntity
  ...
  def make
    opts = @_entopts.dup
    DetokenizeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinShieldconexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Detokenize, Hash] the current Detokenize data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinShieldconexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Detokenize fields)
  def match_get
  ...
  #
  # @param reqmatch [DetokenizeListMatch, Hash, nil] match filter (any subset of
  #   Detokenize fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Detokenize>, Array] the matching Detokenize items; raises BluefinShieldconexError on failure
  ...
  #
  # @param reqdata [DetokenizeCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Detokenize, Hash] the created Detokenize; raises BluefinShieldconexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = BluefinShieldconexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
