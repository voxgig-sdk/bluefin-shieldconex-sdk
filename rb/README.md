# BluefinShieldconex Ruby SDK



The Ruby SDK for the BluefinShieldconex API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Detokenize` — with named operations (`list`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/bluefin-shieldconex-sdk/releases](https://github.com/voxgig-sdk/bluefin-shieldconex-sdk/releases)
...
```ruby
client = BluefinShieldconexSDK.test(testopts, sdkopts)
```
...
raise a `BluefinShieldconexError` (a `StandardError` subclass), so wrap
...
Create an instance: `detokenize = client.Detokenize`
...
```ruby
# list returns an Array of Detokenize records (raises on error).
detokenizes = client.Detokenize.list
```
...
Create an instance: `tokenize_batch = client.TokenizeBatch`
...
```ruby
tokenize_batch = client.TokenizeBatch.create({
})
```
...
Create an instance: `tokenize_read = client.TokenizeRead`
...
| `messageId` | `String` | Message Id |
| `reference` | `String` | Request Reference. |
| `state` | `Hash` | Tokenized State Data (if available) |
| `values` | `Array` |  |

#### Example: Create

```ruby
tokenize_read = client.TokenizeRead.create({
})
```
...
```
rb/
├── BluefinShieldconex_sdk.rb       -- Main SDK module
...
The main module (`BluefinShieldconex_sdk`) exports the SDK class
