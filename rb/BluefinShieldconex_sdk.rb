...
require_relative 'BluefinShieldconex_types'
...
class BluefinShieldconexSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BluefinShieldconexHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BluefinShieldconexHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BluefinShieldconexFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BluefinShieldconexUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BluefinShieldconexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BluefinShieldconexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BluefinShieldconexHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BluefinShieldconexError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue BluefinShieldconexError => err
    ...
    if fetched.is_a?(Hash)
      status = BluefinShieldconexHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BluefinShieldconexError.new(
        "graphql_error", "BluefinShieldconexSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.Detokenize.list / client.Detokenize.load({ "id" => ... })
  def Detokenize(data = nil)
    require_relative 'entity/detokenize_entity'
    DetokenizeEntity.new(self, data)
  end
  ...
  # Canonical facade: client.TokenizeBatch.list / client.TokenizeBatch.load({ "id" => ... })
  def TokenizeBatch(data = nil)
    require_relative 'entity/tokenize_batch_entity'
    TokenizeBatchEntity.new(self, data)
  end


  # Canonical facade: client.TokenizeRead.list / client.TokenizeRead.load({ "id" => ... })
  def TokenizeRead(data = nil)
    require_relative 'entity/tokenize_read_entity'
    TokenizeReadEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BluefinShieldconexSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
