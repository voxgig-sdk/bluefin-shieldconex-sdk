...
module BluefinShieldconexUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinShieldconexResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinShieldconexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinShieldconexResponse.new(fetched)
    else
