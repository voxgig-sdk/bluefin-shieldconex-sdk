# BluefinShieldconex Lean SDK



The Lean SDK for the BluefinShieldconex API — an entity-oriented client
...
[Releases](https://github.com/voxgig-sdk/bluefin-shieldconex-sdk/releases)) or from a source checkout. The runtime is
...
def main : IO Unit := do
  let ctx ← mkCtx
  (do
    let opts ← newMap #[("base", .str "https://api.example.com"), ("apikey", .str (← IO.getEnv "BLUEFIN_SHIELDCONEX").getD "")]
    let sdk ← Sdk.newSdk opts
    let result ← Detokenize.list sdk (← emptyMap) (← emptyMap)
