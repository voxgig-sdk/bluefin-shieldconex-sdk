# bluefin-shieldconex-cli

boru-driven command-line client **and** interactive REPL for the BluefinShieldconex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-shieldconex-cli)
make build

# 2. See usage (words, entities, env vars)
./bluefin-shieldconex-cli --help

# 3. Provide credentials once, via the environment
export BLUEFIN_SHIELDCONEX_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./bluefin-shieldconex-cli list detokenize
./bluefin-shieldconex-cli list tokenize

# 5. Override the API base URL for a single call
BLUEFIN_SHIELDCONEX_BASE=https://api.example.com ./bluefin-shieldconex-cli list detokenize

# 6. No arguments -> interactive REPL
./bluefin-shieldconex-cli
bluefin-shieldconex> list detokenize
bluefin-shieldconex> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-shieldconex-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bluefin-shieldconex-cli list detokenize
   ```
...
Run with no arguments to open a REPL (prompt `bluefin-shieldconex>`). Each line is
evaluated as its own boru expression:

```text
$ ./bluefin-shieldconex-cli
bluefin-shieldconex> list detokenize
bluefin-shieldconex> /help
bluefin-shieldconex> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `detokenize`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bluefin-shieldconex-cli`. |
...
(`./bluefin-shieldconex-cli <expr>`) and interactively (the REPL), and expressions compose the
