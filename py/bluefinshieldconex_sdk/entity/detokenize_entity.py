...
from bluefinshieldconex_sdk.bluefinshieldconex_types import (
    Detokenize,
    DetokenizeListMatch,
...
class DetokenizeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DetokenizeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Detokenize]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Detokenize().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DetokenizeCreateData, ctrl=None) -> Detokenize:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
