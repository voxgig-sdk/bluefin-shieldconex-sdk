...
from bluefinshieldconex_sdk.core.spec import BluefinShieldconexSpec
from bluefinshieldconex_sdk.core import helpers

# Load utility registration (populates Utility._registrar)
from bluefinshieldconex_sdk.utility import register

# Load features
from bluefinshieldconex_sdk.feature.base_feature import BluefinShieldconexBaseFeature
from bluefinshieldconex_sdk.features import _has_feature, _make_feature
...
class BluefinShieldconexSDK:
    ...
    def __init__(self, options=None):
        ...
        utility = BluefinShieldconexUtility()
        self._utility = utility

        from bluefinshieldconex_sdk.config import shared_config
    ...
    def prepare(self, fetchargs=None):
        ...
        ctx.spec = BluefinShieldconexSpec({
            "base": base,
            "prefix": prefix,
            "suffix": suffix,
            "path": path,
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("BluefinShieldconexSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def Detokenize(self, data=None) -> "DetokenizeEntity":
        """Entity factory: client.Detokenize().list() / client.Detokenize().load({"id": ...})."""
        from bluefinshieldconex_sdk.entity.detokenize_entity import DetokenizeEntity
        return DetokenizeEntity(self, data)


    def Tokenize(self, data=None) -> "TokenizeEntity":
        """Entity factory: client.Tokenize().list() / client.Tokenize().load({"id": ...})."""
        from bluefinshieldconex_sdk.entity.tokenize_entity import TokenizeEntity
        return TokenizeEntity(self, data)


    def TokenizeBatch(self, data=None) -> "TokenizeBatchEntity":
        """Entity factory: client.TokenizeBatch().list() / client.TokenizeBatch().load({"id": ...})."""
        from bluefinshieldconex_sdk.entity.tokenize_batch_entity import TokenizeBatchEntity
        return TokenizeBatchEntity(self, data)


    def TokenizeRead(self, data=None) -> "TokenizeReadEntity":
        """Entity factory: client.TokenizeRead().list() / client.TokenizeRead().load({"id": ...})."""
        from bluefinshieldconex_sdk.entity.tokenize_read_entity import TokenizeReadEntity
        return TokenizeReadEntity(self, data)


    def Validate(self, data=None) -> "ValidateEntity":
        """Entity factory: client.Validate().list() / client.Validate().load({"id": ...})."""
        from bluefinshieldconex_sdk.entity.validate_entity import ValidateEntity
        return ValidateEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "BluefinShieldconexSDK":
        if sdkopts is None:
            sdkopts = {}
