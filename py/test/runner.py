# BluefinShieldconex SDK test runner
...
class BluefinShieldconexTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinShieldconexTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinShieldconexTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinShieldconexTestRunner.getenv("BLUEFIN_SHIELDCONEX_TEST_LIVE")
        override = BluefinShieldconexTestRunner.getenv("BLUEFIN_SHIELDCONEX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinShieldconexTestRunner.getenv(key)
        ...
        explain = BluefinShieldconexTestRunner.getenv("BLUEFIN_SHIELDCONEX_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinShieldconexTestRunner._test_control is not None:
            return BluefinShieldconexTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinShieldconexTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinShieldconexTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinShieldconexTestRunner.load_test_control()
...
def env_override(m):
    return BluefinShieldconexTestRunner.env_override(m)


def entity_data(v):
    return BluefinShieldconexTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinShieldconexTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinShieldconexTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinShieldconexTestRunner.load_test_control()


def live_client_options():
    return BluefinShieldconexTestRunner.live_client_options()


def live_delay_ms():
    return BluefinShieldconexTestRunner.live_delay_ms()
