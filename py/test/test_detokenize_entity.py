    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BLUEFIN_SHIELDCONEX_TEST_DETOKENIZE_ENTID JSON to run live")
        client = setup["client"]
        ...
        detokenize_ref01_data = helpers.to_map(runner.entity_data(detokenize_ref01_ent.create(detokenize_ref01_data, None)))
...
def _detokenize_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BLUEFIN_SHIELDCONEX_TEST_DETOKENIZE_ENTID"))
