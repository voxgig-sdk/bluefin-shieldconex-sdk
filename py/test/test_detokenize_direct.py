...
class TestDetokenizeDirect:

    def test_should_direct_list_detokenize(self):
        setup = _detokenize_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-detokenize", "live" if setup["live"] else "unit")
        if _skip:
...
def _detokenize_direct_setup(mockres):
