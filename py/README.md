# BluefinShieldconex Python SDK



The Python SDK for the BluefinShieldconex API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/bluefin-shieldconex-sdk/releases)) or
...
```python
try:
    detokenizes = client.Detokenize().list({"bfid": "example"})
...
```python
tokenizes = client.Tokenize().list({"template_ref": "example"})
```
...
```
py/
├── bluefinshieldconex_sdk.py         -- Main SDK module
