// Tokenize entity client (generated — mirrors the go Entity fragment).
...
impl TokenizeEntity {
    pub fn new(client: &Rc<BluefinShieldconexSDK>, entopts: Value) -> Rc<TokenizeEntity> {
...
impl Entity for TokenizeEntity {
    ...
    fn make(&self) -> Rc<dyn Entity> {
        ...
        TokenizeEntity::new(&self.client, opts) as Rc<dyn Entity>
    }
...
impl BluefinShieldconexEntity for TokenizeEntity {
    ...
    fn list(self: &Rc<Self>, reqmatch: Value, ctrl: Value) -> Result<Vec<Rc<Self>>, BluefinShieldconexError> {
        ...
        if let Value::List(list) = &out {
            for entry in list.borrow().iter() {
                let ent = TokenizeEntity::new(&self.client, vs::clone(&self.entopts));
