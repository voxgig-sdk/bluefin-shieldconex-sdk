...
fn tokenize_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEX_TEST_TOKENIZE_ENTID JSON to run live");
        return;
    }
    ...
    let tokenize_ref01_data = to_map(&tokenize_ref01_data_result);
    assert!(
    ...
    let tokenize_ref01_list = tokenize_ref01_ent
        ...
        .expect("list failed");
    assert!(
        matches!(tokenize_ref01_list, Value::List(_)),
...
fn tokenize_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFINSHIELDCONEX_TEST_TOKENIZE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEX_TEST_TOKENIZE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINSHIELDCONEX_TEST_TOKENIZE_ENTID")) {
