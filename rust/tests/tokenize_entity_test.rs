...
#[test]
fn tokenize_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.tokenize(Value::Noval);
    assert_eq!(ent.get_name(), "tokenize");
}

#[test]
fn tokenize_entity_stream() {
    ...
    let plainent = plainsdk.tokenize(Value::Noval);
...
#[test]
fn tokenize_entity_basic() {
    let setup = tokenize_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create", "list"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("tokenize.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFIN_SHIELDCONEX_TEST_TOKENIZE_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let tokenize_ref01_ent = client.tokenize(Value::Noval);
    let tokenize_ref01_data = to_map(&getp(
        &getpath(&["new", "tokenize"], &setup.data),
        "tokenize_ref01",
    ));

    let tokenize_ref01_data_result = tokenize_ref01_ent
        .create(tokenize_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let tokenize_ref01_data = to_map(&tokenize_ref01_data_result.data(None));
    assert!(
        matches!(tokenize_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

    // LIST
    let tokenize_ref01_match = Value::empty_map();

    let tokenize_ref01_list = tokenize_ref01_ent
        .list(tokenize_ref01_match.clone(), Value::Noval)
        .expect("list failed");
    // list resolves to one ENTITY per record; the flow asserts on the
    // records, so map each through data().
    let tokenize_ref01_list = ja(tokenize_ref01_list.iter().map(|e| e.data(None)).collect::<Vec<Value>>());

    let found_item = vs::select(
        &entity_list_to_data(&tokenize_ref01_list),
        &jo(vec![("id", getp(&tokenize_ref01_data, "id"))]),
    );
    assert!(
...
fn tokenize_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("tokenize");
    entity_data_file.push("TokenizeTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("tokenize01"), Value::str("tokenize02"), Value::str("tokenize03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFIN_SHIELDCONEX_TEST_TOKENIZE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFIN_SHIELDCONEX_TEST_TOKENIZE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFIN_SHIELDCONEX_TEST_TOKENIZE_ENTID")) {
