...
#[test]
fn tokenize_batch_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.tokenize_batch(Value::Noval);
    assert_eq!(ent.get_name(), "tokenize_batch");
}

#[test]
fn tokenize_batch_entity_stream() {
    ...
    let plainent = plainsdk.tokenize_batch(Value::Noval);
...
#[test]
fn tokenize_batch_entity_basic() {
    let setup = tokenize_batch_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("tokenize_batch.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEX_TEST_TOKENIZE_BATCH_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let tokenize_batch_ref01_ent = client.tokenize_batch(Value::Noval);
    let tokenize_batch_ref01_data = to_map(&getp(
        &getpath(&["new", "tokenize_batch"], &setup.data),
        "tokenize_batch_ref01",
    ));

    let tokenize_batch_ref01_data_result = tokenize_batch_ref01_ent
        .create(tokenize_batch_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let tokenize_batch_ref01_data = to_map(&tokenize_batch_ref01_data_result);
    assert!(
        matches!(tokenize_batch_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn tokenize_batch_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("tokenize_batch");
    entity_data_file.push("TokenizeBatchTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("tokenize_batch01"), Value::str("tokenize_batch02"), Value::str("tokenize_batch03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINSHIELDCONEX_TEST_TOKENIZE_BATCH_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEX_TEST_TOKENIZE_BATCH_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINSHIELDCONEX_TEST_TOKENIZE_BATCH_ENTID")) {
