...
#[test]
fn validate_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.validate(Value::Noval);
    assert_eq!(ent.get_name(), "validate");
}

#[test]
fn validate_entity_stream() {
    ...
    let plainent = plainsdk.validate(Value::Noval);
...
#[test]
fn validate_entity_basic() {
    let setup = validate_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("validate.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEX_TEST_VALIDATE_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let validate_ref01_ent = client.validate(Value::Noval);
    let validate_ref01_data = to_map(&getp(
        &getpath(&["new", "validate"], &setup.data),
        "validate_ref01",
    ));

    let validate_ref01_data_result = validate_ref01_ent
        .create(validate_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let validate_ref01_data = to_map(&validate_ref01_data_result);
    assert!(
        matches!(validate_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn validate_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("validate");
    entity_data_file.push("ValidateTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("validate01"), Value::str("validate02"), Value::str("validate03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINSHIELDCONEX_TEST_VALIDATE_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINSHIELDCONEX_TEST_VALIDATE_ENTID", idmap.clone()),
    ...
    let idmap_resolved = match to_map(&getp(&env, "BLUEFINSHIELDCONEX_TEST_VALIDATE_ENTID")) {
