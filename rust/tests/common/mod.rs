...
/// env_override (mirrors go): when BLUEFIN_SHIELDCONEX_TEST_LIVE or
/// BLUEFIN_SHIELDCONEX_TEST_OVERRIDE is TRUE, environment variables replace the
/// defaults in `m` (JSON-shaped values are parsed).
pub fn env_override(m: Value) -> Value {
    let live = std::env::var("BLUEFIN_SHIELDCONEX_TEST_LIVE").unwrap_or_default() == "TRUE";
    let over = std::env::var("BLUEFIN_SHIELDCONEX_TEST_OVERRIDE").unwrap_or_default() == "TRUE";
    ...
    if let Ok(explain) = std::env::var("BLUEFIN_SHIELDCONEX_TEST_EXPLAIN") {
        if !explain.is_empty() {
            setp(&m, "BLUEFIN_SHIELDCONEX_TEST_EXPLAIN", Value::str(explain));
        }
    }
...
pub struct EntityTestSetup {
    pub client: Rc<BluefinShieldconexSDK>,
    pub data: Value,
    pub idmap: Value,
    pub env: Value,
    pub explain: bool,
...
pub fn match_string(pattern: &str, val: &str) -> bool {
    if pattern.len() >= 2 && pattern.starts_with('/') && pattern.ends_with('/') {
        return bluefin_shieldconex_sdk::utility::voxgigstruct::re::Regex::new(
...
pub fn runset(
    testspec: &Value,
    subject: &mut dyn FnMut(&Value) -> Result<Value, BluefinShieldconexError>,
) {
    ...
    if let Value::Map(_) = getp(&ctxmap, "result") {
        ...
        if let Value::Map(_) = getp(&res_map, "err") {
            if let Some(msg) = get_str(&getp(&res_map, "err"), "message") {
                result.borrow_mut().err = Some(BluefinShieldconexError::new("", &msg));
            }
        }
...
pub fn fixctx(ctx: &Rc<Context>, client: &Rc<BluefinShieldconexSDK>) {
    if ctx.options.borrow().is_noval() {
        *ctx.options.borrow_mut() = client.options_map();
    }
}
...
pub fn err_from_map(m: &Value) -> Option<BluefinShieldconexError> {
    let msg = get_str(m, "message").unwrap_or_default();
    if msg.is_empty() {
        return None;
    }
    let code = get_str(m, "code").unwrap_or_default();
    Some(BluefinShieldconexError::new(&code, &msg))
}
...
pub fn fh_has_feature(name: &str) -> bool {
    let config = bluefin_shieldconex_sdk::make_config();
...
pub type FhReply = Rc<dyn Fn(i64, &Value) -> Result<Value, BluefinShieldconexError>>;
...
pub struct FhOpResult {
    pub ok: bool,
    pub data: Value,
    pub err: Option<BluefinShieldconexError>,
...
fn fh_populate_result(ctx: &Rc<Context>, fetched: &Result<Value, BluefinShieldconexError>) {
    ...
    *ctx.result.borrow_mut() = Some(result.clone());
    ...
    let resp = Response::new(response_map);
    {
        ...
        if let Value::Func(_) = &resp.json {
            r.body = bluefin_shieldconex_sdk::call_json(&resp.json);
        }
...
impl FhHarness {
    ...
    pub fn op(&self, o: FhOpSpec) -> FhOpResult {
        ...
        *ctx.spec.borrow_mut() = Some(spec.clone());
        ...
        let fetched: Result<Value, BluefinShieldconexError> = match ctx.out_get("request") {
    ...
    fn fail(&self, ctx: Rc<Context>, err: BluefinShieldconexError) -> FhOpResult {
        {
...
pub fn fh_err_code(err: &Option<BluefinShieldconexError>) -> String {
