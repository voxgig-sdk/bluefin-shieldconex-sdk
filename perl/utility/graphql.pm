# BluefinShieldconex SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinShieldconexHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinShieldconexHelpers::gp($ext, 'code');
  $raw = BluefinShieldconexHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinShieldconexHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinShieldconexHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinShieldconexHelpers::gp($spec, 'name');
    my $from = BluefinShieldconexHelpers::gp($spec, 'from');
    ...
    my $val = BluefinShieldconexHelpers::gp($reqsrc, $from);
    $val = BluefinShieldconexHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinShieldconexHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinShieldconexHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinShieldconexHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinShieldconexHelpers::gp($first, 'message');
