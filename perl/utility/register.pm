...
$BluefinShieldconexUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinShieldconexUtilities::REGISTRY) {
    $u->{$k} = $BluefinShieldconexUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinShieldconexHelpers::struct_facade();
};
