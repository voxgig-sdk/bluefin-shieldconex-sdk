# BluefinShieldconex SDK utility: make_point
...
sub _parts_len {
  my ($point) = @_;
  my $parts = BluefinShieldconexHelpers::gp($point, 'parts');
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinShieldconexError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinShieldconexHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    my $matched = 0;
    for my $p (@{ $op->{points} }) {
      my $select_def = BluefinShieldconexHelpers::to_map(BluefinShieldconexHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinShieldconexHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinShieldconexHelpers::gp($reqselector, "$ek");
            my $sv = BluefinShieldconexHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinShieldconexHelpers::gp($reqselector, '$action');
        my $select_action = BluefinShieldconexHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinShieldconexHelpers::eqv($req_action, $select_action);
      }
    ...
    unless ($matched) {
      ...
      my $unmatched_action = $reqselector
        ? BluefinShieldconexHelpers::gp($reqselector, '$action') : undef;
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinShieldconexHelpers::to_map(BluefinShieldconexHelpers::gp($point, 'select'));
        my $point_action = BluefinShieldconexHelpers::gp($point_select, '$action');
        unless (BluefinShieldconexHelpers::eqv($req_action, $point_action)) {
          return (undef, $ctx->make_error('point_action_invalid',
            "Operation \"$op->{name}\" action \""
