...
sub new {
  ...
  my $config = BluefinShieldconexConfig::shared_config();
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = BluefinShieldconexHelpers::gpath($self->{options}, 'allow.op');
...
sub graphql {
  ...
  my $errors = BluefinShieldconexHelpers::gpath($res, 'data.errors');
