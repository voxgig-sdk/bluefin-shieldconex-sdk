# BluefinShieldconex SDK TokenizeBatch entity
...
sub make {
  ...
  return TokenizeBatchEntity->new($self->{_client}, $opts);
}
...
# Returns the current TokenizeBatch data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TokenizeBatch fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TokenizeBatch.
#
# reqdata: body data hashref (TokenizeBatchCreateData shape). ctrl: optional
# per-call control. Returns the created TokenizeBatch data (hashref); dies
