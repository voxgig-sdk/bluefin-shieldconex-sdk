...
sub new {
  ...
  elsif (BluefinShieldconexHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
...
sub make {
  ...
  return DetokenizeEntity->new($self->{_client}, $opts);
}

sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinShieldconexHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current Detokenize data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinShieldconexHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of Detokenize fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match filter hashref (any subset of Detokenize fields;
# DetokenizeListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Detokenize items as an arrayref; dies with
# BluefinShieldconexError on failure.
sub list {
...
#
# reqdata: body data hashref (DetokenizeCreateData shape). ctrl: optional
# per-call control. Returns the created Detokenize data (hashref); dies
# with BluefinShieldconexError on failure.
sub create {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      if ($result->{resdata}) {
        $self->{_data} = BluefinShieldconexHelpers::to_map(
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinShieldconexHelpers::to_map($callopts) || {};
  my $signal = BluefinShieldconexHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinShieldconexHelpers::to_map(
    BluefinShieldconexHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinShieldconexHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinShieldconexHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinShieldconexHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
