# BluefinShieldconex SDK TokenizeRead entity
...
sub make {
  ...
  return TokenizeReadEntity->new($self->{_client}, $opts);
}
...
# Returns the current TokenizeRead data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of TokenizeRead fields).
sub match_get {
  my ($self) = @_;
...
# Create a new TokenizeRead.
#
# reqdata: body data hashref (TokenizeReadCreateData shape). ctrl: optional
# per-call control. Returns the created TokenizeRead data (hashref); dies
