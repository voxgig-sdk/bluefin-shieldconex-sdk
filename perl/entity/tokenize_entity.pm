...
# Returns the current Tokenize data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of Tokenize fields).
sub match_get {
  my ($self) = @_;
...
#
# reqmatch: match filter hashref (any subset of Tokenize fields;
# TokenizeListMatch shape); defaults to undef, treated as an empty match
# that lists all. ctrl: optional per-call control.
# Returns the matching Tokenize items as an arrayref; dies with
...
# Create a new Tokenize.
#
# reqdata: body data hashref (TokenizeCreateData shape). ctrl: optional
# per-call control. Returns the created Tokenize data (hashref); dies
