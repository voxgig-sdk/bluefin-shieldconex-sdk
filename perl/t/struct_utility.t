...
my $runner = BluefinShieldconexOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinShieldconexOmni::NULLMARK();
...
{
  my $td = BluefinShieldconexOmni::tostruct(
...
{
  my $basic = BluefinShieldconexOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinShieldconexOmni::null_modifier }) });
