...
sub resp {
  ...
  return BluefinShieldconexResponse->new({
    'status' => $status,
...
sub spec_of {
  my (%map) = @_;
  return BluefinShieldconexSpec->new({
...
sub is_sdk_error {
  my ($err) = @_;
  return Scalar::Util::blessed($err) && $err->isa('BluefinShieldconexError');
}
...

{
  ...
  $ctx->{spec} = spec_of();
  $ctx->{result} = BluefinShieldconexResult->new({ 'ok' => 1, 'resdata' => [] });
  my ($result, $err) = $utility->{make_result}->($ctx);
...
{
  my $ctx = make_ctx();
  my $preset = BluefinShieldconexResult->new({ 'ok' => 1 });
...
{
  my $boom = BluefinShieldconexError->new('boom', 'boom');
...

{
  ...
  $u->{make_fetch_def} = sub {
    return (undef, BluefinShieldconexError->new('fetchdef_boom', 'boom'));
  };
  my $ctx = make_ctx(utility => $u);
  $ctx->{spec} = request_spec();
  my ($response, $err) = $u->{make_request}->($ctx);
...
{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinShieldconexResult->new({ 'ok' => 1, 'resdata' => 42 });
  is($utility->{done}->($ctx), 42, 'done returns resdata on success');
}

{
  my $ctx = make_ctx();
  $ctx->{result} = BluefinShieldconexResult->new({ 'ok' => 0 });
...

{
  ...
  $ctx->{result} = BluefinShieldconexResult->new({ 'ok' => 1, 'resdata' => 7 });
...

{
  ...
  $ctx->{result} = BluefinShieldconexResult->new({ 'ok' => 0, 'resdata' => 'fallback' });
...
{
  my $oclient = BluefinShieldconexSDK->test(undef, undef);
  ...
  $oclient->{features} = [];
  my $a = BluefinShieldconexBaseFeature->new;
  my $b = BluefinShieldconexBaseFeature->new;
...
sub named_feature {
  my ($name) = @_;
  my $f = BluefinShieldconexBaseFeature->new;
  $f->{name} = $name;
  return $f;
}
...

{
  ...
  ok(BluefinShieldconexHelpers::is_true($o2->{feature}{metrics}{active}),
    'feature order: array normalized to a map (metrics opts preserved)');
  ok(BluefinShieldconexHelpers::is_true($o2->{feature}{test}{active}),
...
sub auth_ctx {
  ...
  my $ctx = BluefinShieldconexContext->new({
    'client' => PipelineOptClient->new($options),
    'utility' => $utility,
    'opname' => 'load',
  }, undef);
  $ctx->{spec} = defined $headers
    ? BluefinShieldconexSpec->new({ 'headers' => $headers, 'step' => 's' })
    : undef;
  return $ctx;
}
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinShieldconexResponse->new({ 'status' => 200 });
  $ctx->{result} = BluefinShieldconexResult->new({});
...
{
  my $ctx = make_ctx();
  $ctx->{response} = BluefinShieldconexResponse->new({
