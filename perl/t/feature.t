...
# operation pipeline. $features is a list of { name =>, options => }.
{
  ...
  sub new {
    ...
    $self->{utility} = BluefinShieldconexUtility->new;
    ...
    for my $fspec (@$features) {
      ...
      my $f = BluefinShieldconexFeatures::make_feature($name);
  ...
  sub op {
    ...
    my $out = eval {
      ...
      if (Scalar::Util::blessed($ctx->{out}{point})
        && $ctx->{out}{point}->isa('BluefinShieldconexError')) {
        die $ctx->{out}{point};
      }
      ...
      $ctx->{response} = Voxgig::Struct::ismap($fetched)
        ? BluefinShieldconexResponse->new($fetched) : undef;
    ...
    if (my $err = $@) {
      if (Scalar::Util::blessed($err) && $err->isa('BluefinShieldconexError')) {
        $ctx->{ctrl}{err} = $err;
  ...
  sub populate_result {
    ...
    my $result = BluefinShieldconexResult->new({});
    $ctx->{result} = $result;
...
if (has_feature('retry')) {
...

  {
    ...
    my ($server, $calls) = recording_server(sub {
      my ($n) = @_;
      return $n < 3
        ? (undef, BluefinShieldconexError->new('boom', 'boom'))
