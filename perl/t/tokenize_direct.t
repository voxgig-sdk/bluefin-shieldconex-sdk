...
DIRECT_LIST: {
  my $setup = tokenize_direct_setup([
    { 'id' => 'direct01' },
    { 'id' => 'direct02' },
  ]);
  my ($_should_skip, $_reason) = BluefinShieldconexTestRunner::is_control_skipped(
    'direct', 'direct-list-tokenize', $setup->{live} ? 'live' : 'unit');
  if ($_should_skip) {
    note($_reason || 'skipped via sdk-test-control.json');
    pass('direct-list-tokenize: skipped via sdk-test-control.json');
    last DIRECT_LIST;
  }
  ...
  if ($setup->{live}) {
    ...
    if (defined $result->{err}) {
      ...
      pass('direct-list-tokenize: skipped (live)');
      last DIRECT_LIST;
    }
    unless ($result->{ok}) {
    ...
    pass('direct-list-tokenize: live ok');
  }
  else {
    ok(!defined $result->{err}, 'direct-list-tokenize: no error');
    ok($result->{ok}, 'direct-list-tokenize: ok');
    is(BluefinShieldconexHelpers::to_int($result->{status}), 200, 'direct-list-tokenize: status');
    ok(Voxgig::Struct::islist($result->{data}), 'direct-list-tokenize: data is array');
    is(scalar @{ $result->{data} }, 2, 'direct-list-tokenize: data length');
    is(scalar @{ $setup->{calls} }, 1, 'direct-list-tokenize: 1 call');
  }
}


sub tokenize_direct_setup {
  my ($mockres) = @_;
