...
sub has_feature {
  my ($name) = @_;
  my $f = BluefinShieldconexConfig::make_config()->{feature};
...
sub find_list_entity {
  ...
  my $entities = BluefinShieldconexHelpers::to_map(
    BluefinShieldconexHelpers::gp($config, 'entity')) || {};
  for my $name (sort keys %$entities) {
    my $points = BluefinShieldconexHelpers::gpath($config, "entity.$name.op.list.points");
    next unless Voxgig::Struct::islist($points);
    for my $p (@$points) {
      my $params = BluefinShieldconexHelpers::gpath($p, 'args.params');
      my $reqd = Voxgig::Struct::islist($params)
        ? scalar(grep { BluefinShieldconexHelpers::is_true($_->{reqd}) } @$params)
        : 0;
...
SKIP: {
  ...
  {
    my $sdk = BluefinShieldconexSDK->test({ 'entity' => $seed }, undef);
  ...
  if (has_feature('streaming')) {
    {
      my $sdk = BluefinShieldconexSDK->test(
        { 'entity' => $seed },
        { 'feature' => { 'streaming' => {
