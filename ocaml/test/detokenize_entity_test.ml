(* Generated detokenize entity test. *)
...
let () =
  test "detokenize.entity_instance" (fun () ->
      let client = Sdk_client.test () in
      let ent = Sdk_client.detokenize client Noval in
      check_str "name" ent.e_name "detokenize")

let () =
  test "detokenize.seeded_ops" (fun () ->
      let record = jo [("id", Str "detokenize01")] in
      let seed = jo [("detokenize",
                      jo [("detokenize01", record)])] in
...
let () =
  test "detokenize.stream" (fun () ->
      ...
      let ent2 = Sdk_client.detokenize client2 Noval in
      let n = ref 0 in
      ...
      if has_streaming then begin
        ...
        let sent = Sdk_client.detokenize sclient Noval in
        ...
        let cent = Sdk_client.detokenize cclient Noval in
