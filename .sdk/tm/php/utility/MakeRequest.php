...
class BluefinShieldconexMakeRequest
{
    public static function call(BluefinShieldconexContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new BluefinShieldconexResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new BluefinShieldconexResponse($fetched);
        } else {
