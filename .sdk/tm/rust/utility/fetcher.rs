...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinShieldconexError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinShieldconexSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinShieldconexError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinShieldconexError::new("fetch_body", &format!("{}", e)))?;
