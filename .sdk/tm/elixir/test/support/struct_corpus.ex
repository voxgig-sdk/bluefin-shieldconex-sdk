# Vendored struct corpus runner for the BluefinShieldconex SDK.
#
...
defmodule BluefinShieldconex.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinShieldconex.test(nil, S.clone(setup)), else: BluefinShieldconex.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinShieldconex.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinShieldconex.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinShieldconex.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinShieldconex.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
