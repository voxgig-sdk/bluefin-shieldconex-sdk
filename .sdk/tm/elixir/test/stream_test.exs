# BluefinShieldconex SDK entity stream() test
#
...
defmodule BluefinShieldconex.StreamTest do
  use ExUnit.Case
  ...
  defp has_feature?(name) do
    f = S.getprop(BluefinShieldconex.Config.make_config(), "feature")
  ...
  defp entity(sdk, name), do: apply(BluefinShieldconex, String.to_atom(name), [sdk, nil])
  defp entmod(name), do: Module.concat([BluefinShieldconex.Entity, Macro.camelize(name)])
  ...
  test "entity stream() runs the pipeline and yields items" do
    ...
    if name == nil do
      ...
      sdk = BluefinShieldconex.test(S.jm(["entity", seed(name)]), nil)
      items = do_stream(sdk, name, nil)
      assert length(items) == 3
      assert S.ismap(hd(items))

      # signal cancels iteration between yields.
      sdk2 = BluefinShieldconex.test(S.jm(["entity", seed(name)]), nil)
