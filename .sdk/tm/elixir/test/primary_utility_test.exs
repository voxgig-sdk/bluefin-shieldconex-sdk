...
defmodule BluefinShieldconex.PrimaryUtilityTest do
  ...
  alias BluefinShieldconex.{Context, Utility, Spec, Result}

  defp client, do: BluefinShieldconex.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinShieldconex.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinShieldconex.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinShieldconex.test()
    opts = BluefinShieldconex.options_map(c)
