# BluefinShieldconex SDK paging feature
#
...
package BluefinShieldconexPagingFeature;
...
sub init {
  ...
  $self->{active} = BluefinShieldconexHelpers::is_true($self->{options}{active});
  return;
}
...
sub PreRequest {
  ...
  if ('graphql' eq (BluefinShieldconexHelpers::gp($ctx->{point}, 'kind') // '')) {
...
sub _graphql_pre_request {
  ...
  my %declared;
  my $varlist = BluefinShieldconexHelpers::gpath($ctx->{point}, 'graphql.vars');
  if (ref($varlist) eq 'ARRAY') {
    for my $v (@$varlist) {
      my $name = BluefinShieldconexHelpers::gp($v, 'name');
...
sub PreResult {
  ...
  my $page = BluefinShieldconexHelpers::gpath($ctx->{point}, 'graphql.page');
  if (ref($page) eq 'HASH' && Voxgig::Struct::ismap($body)) {
    ...
    if (defined $connpath && '' ne $connpath) {
      my $sub = BluefinShieldconexHelpers::gpath($body, $connpath);
      $conn = $sub if defined $sub;
    }

    my $cursorpath = $page->{cursor};
    if (defined $cursorpath && '' ne $cursorpath) {
      my $cursor = BluefinShieldconexHelpers::gpath($conn, $cursorpath);
      $paging->{cursor} = $cursor if defined $cursor;
    }

    my $morepath = $page->{more};
    if (defined $morepath && '' ne $morepath) {
      my $more = BluefinShieldconexHelpers::gpath($conn, $morepath);
      if (BluefinShieldconexHelpers::is_true($more) || BluefinShieldconexHelpers::is_false($more)) {
        $paging->{hasMore} = BluefinShieldconexHelpers::is_true($more) ? 1 : 0;
        $explicit_more = 1;
      }
    }
  }
  ...
  if (Voxgig::Struct::ismap($body)) {
    my $bnext = BluefinShieldconexHelpers::gp($body, 'next');
    $paging->{next} = defined $paging->{next} ? $paging->{next} : $bnext
      if defined $bnext;
    my $bcursor = BluefinShieldconexHelpers::gp($body, 'cursor');
    $paging->{cursor} = $bcursor if defined $bcursor;
    my $bnextcursor = BluefinShieldconexHelpers::gp($body, 'nextCursor');
    $paging->{cursor} = $bnextcursor if defined $bnextcursor;
    my $bhasmore = $body->{hasMore};
    if (BluefinShieldconexHelpers::is_true($bhasmore) || BluefinShieldconexHelpers::is_false($bhasmore)) {
      $paging->{hasMore} = BluefinShieldconexHelpers::is_true($bhasmore) ? 1 : 0;
      $explicit_more = 1;
    }
  }
