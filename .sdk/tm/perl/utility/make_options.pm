# BluefinShieldconex SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinShieldconexHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinShieldconexHelpers::gp($options, 'extend');
  ...
  my @featureorder;
  my $feature_raw = BluefinShieldconexHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinShieldconexHelpers::gp($entry, 'name');
  ...
  BluefinShieldconexHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinShieldconexHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinShieldconexConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinShieldconexHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinShieldconexHelpers::to_map(
      BluefinShieldconexHelpers::gp($opts, 'feature')) || {};
