# BluefinShieldconex SDK utility: make_spec
...
$REGISTRY{make_spec} = sub {
  ...
  my $base = BluefinShieldconexHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = BluefinShieldconexHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = BluefinShieldconexHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = BluefinShieldconexHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = BluefinShieldconexHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = BluefinShieldconexHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $BluefinShieldconexUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
