...
{
  my $sdk = BluefinShieldconexSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinShieldconexSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinShieldconexSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinShieldconexSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinShieldconexSDK->test(undef, undef);
