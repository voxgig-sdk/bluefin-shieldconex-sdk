#!perl
# BluefinShieldconex SDK primary utility test
#
...
#   live blessed context with BluefinShieldconexOmni::livectx before calling a
...
my $_runner = BluefinShieldconexOmni::make_runner($TEST_JSON, BluefinShieldconexSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinShieldconexBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinShieldconexBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinShieldconexError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinShieldconexOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinShieldconexOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinShieldconexResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinShieldconexBaseFeature->new;
...
{
  my $hook_client = BluefinShieldconexSDK->test(undef, undef);
...
{
  my $init_client = BluefinShieldconexSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinShieldconexSDK->new({
...
{
  my $blocked_client = BluefinShieldconexSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinShieldconexOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinShieldconexSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinShieldconexSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makePoint', sub {
  my ($view) = @_;
  my ($point, $err) = $utility->{make_point}->(BluefinShieldconexOmni::livectx($view));
  # The corpus asserts refusals by code (`match: {out: {code: ...}}`), so
  # an error is the RESULT here, flattened to the map the corpus reads.
  return BluefinShieldconexOmni::errify($err) if defined $err;
  return $point;
});
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinShieldconexOmni::livectx($view);
  $ctx->{result} = BluefinShieldconexResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinShieldconexOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinShieldconexOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinShieldconexSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinShieldconexOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinShieldconexOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinShieldconexOmni::livectx($view));
});
