...
#    `contextify` answers a TIED plain hash (BluefinShieldconexOmniViewTie) whose
...
package BluefinShieldconexOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinShieldconexOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinShieldconexError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinShieldconexResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinShieldconexOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinShieldconexOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinShieldconexOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinShieldconexOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinShieldconexOmniProviderTie;
...
package BluefinShieldconexOmniProvider;
...
# The resolver proper.
# --------------------------------------------------------------------------
package BluefinShieldconexOmni;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinShieldconexOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinShieldconexOmniViewTie') ? 1 : 0;
}
...
sub enrich {
  ...
  if (Voxgig::Struct::ismap($ctxmap->{spec}) && !Scalar::Util::blessed($ctxmap->{spec})) {
    $ctx->{spec} = BluefinShieldconexSpec->new($ctxmap->{spec});
  }

  my $res_map = $ctxmap->{result};
  if (Voxgig::Struct::ismap($res_map) && !Scalar::Util::blessed($res_map)) {
    $ctx->{result} = BluefinShieldconexResult->new($res_map);
    if (Voxgig::Struct::ismap($res_map->{err})
      && defined $res_map->{err}{message} && !ref $res_map->{err}{message}) {
      $ctx->{result}{err} = BluefinShieldconexError->new('', $res_map->{err}{message});
    }
  }

  my $resp_map = $ctxmap->{response};
  if (Voxgig::Struct::ismap($resp_map) && !Scalar::Util::blessed($resp_map)) {
    $ctx->{response} = BluefinShieldconexResponse->new($resp_map);
    if (BluefinShieldconexHelpers::rb_truthy($resp_map->{body})) {
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinShieldconexHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinShieldconexOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinShieldconexOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinShieldconexContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinShieldconexOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinShieldconexOmniProvider';
}
