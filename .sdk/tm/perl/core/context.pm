...
package BluefinShieldconexContext;
...
sub new {
  ...
  my $gcp = \&BluefinShieldconexHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinShieldconexControl->new();
  ...
  $self->{data} = BluefinShieldconexHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinShieldconexHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinShieldconexHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinShieldconexHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinShieldconexSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinShieldconexResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinShieldconexResponse'))
...
sub resolve_op {
  ...
  return BluefinShieldconexOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinShieldconexHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinShieldconexHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinShieldconexOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinShieldconexError->new($code, $msg, $self);
}
