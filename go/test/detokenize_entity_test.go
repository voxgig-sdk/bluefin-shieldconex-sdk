...
func TestDetokenizeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		detokenizeRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "detokenize"}), "detokenize_ref01"))

		detokenizeRef01DataResult, err := detokenizeRef01Ent.Create(detokenizeRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		detokenizeRef01Data = core.ToMapAny(entityData(detokenizeRef01DataResult))
...
func detokenizeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_SHIELDCONEX_TEST_DETOKENIZE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_SHIELDCONEX_TEST_DETOKENIZE_ENTID"])
