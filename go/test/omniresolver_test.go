...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-shieldconex-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinShieldconexError); is {
				out := map[string]any{
					"name":    "BluefinShieldconexError",
...
func omniCtx(arg any, client *sdk.BluefinShieldconexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinShieldconexSDK); is {
			client = live
