...
func TestTokenizeReadEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.TokenizeRead(nil)
		if ent == nil {
			t.Fatal("expected non-nil TokenizeReadEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := tokenize_readBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "tokenize_read." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set BLUEFINSHIELDCONEX_TEST_TOKENIZE_READ_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tokenizeReadRef01Data = core.ToMapAny(tokenizeReadRef01DataResult)
		if tokenizeReadRef01Data == nil {
...
func tokenize_readBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "tokenize_read", "TokenizeReadTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read tokenize_read test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse tokenize_read test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("BLUEFINSHIELDCONEX_TEST_TOKENIZE_READ_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFINSHIELDCONEX_TEST_TOKENIZE_READ_ENTID"])
