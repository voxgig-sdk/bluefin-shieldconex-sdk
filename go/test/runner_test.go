...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUEFINSHIELDCONEX_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUEFINSHIELDCONEX_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUEFINSHIELDCONEX_TEST_EXPLAIN"); explain != "" {
