...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUEFIN_SHIELDCONEX_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUEFIN_SHIELDCONEX_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUEFIN_SHIELDCONEX_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.BluefinShieldconexSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.BluefinShieldconexError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.BluefinShieldconexSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.BluefinShieldconexError{Msg: msg, Code: code}
}
