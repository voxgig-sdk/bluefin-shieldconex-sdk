...
type TokenizeReadEntity struct {
	name    string
...
func NewTokenizeReadEntity(client *core.BluefinShieldconexSDK, entopts map[string]any) *TokenizeReadEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *TokenizeReadEntity) GetName() string { return e.name }
...
func (e *TokenizeReadEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *TokenizeReadEntity) Deleted() bool {
	return e.deleted
}


func (e *TokenizeReadEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewTokenizeReadEntity(e.client, opts)
}

func (e *TokenizeReadEntity) Data(args ...any) any {
...
func (e *TokenizeReadEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an TokenizeRead; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *TokenizeReadEntity) DataTyped(data ...TokenizeRead) TokenizeRead {
	if len(data) > 0 {
		return typedFrom[TokenizeRead](e.Data(asMap(data[0])))
	}
	return typedFrom[TokenizeRead](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through TokenizeRead (all fields
// optional at the wire level).
func (e *TokenizeReadEntity) MatchTyped(match ...TokenizeRead) TokenizeRead {
	if len(match) > 0 {
		return typedFrom[TokenizeRead](e.Match(asMap(match[0])))
	}
	return typedFrom[TokenizeRead](e.Match())
}
...
func (e *TokenizeReadEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *TokenizeReadEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *TokenizeReadEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}



func (e *TokenizeReadEntity) Create(reqdata map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// TokenizeReadCreateData and returns an TokenizeRead. It delegates to the untyped
// Create (identical runtime) and converts at the typed boundary.
func (e *TokenizeReadEntity) CreateTyped(reqdata TokenizeReadCreateData, ctrl map[string]any) (TokenizeRead, error) {
	res, err := e.Create(asMap(reqdata), ctrl)
	if err != nil {
		return TokenizeRead{}, err
	}
	return typedFrom[TokenizeRead](res), nil
}



func (e *TokenizeReadEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *TokenizeReadEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *TokenizeReadEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
