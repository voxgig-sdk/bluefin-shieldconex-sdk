// Generated direct-call test for the tokenize entity (mirrors the
...
static voxgig_value* tokenize_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinShieldconexSDK* tokenize_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(tokenize_mock, mockres)));
  return bluefinshieldconex_sdk_new(opts);
}
...
int main(void) {
  ...
  // LIST
  {
    ...
    BluefinShieldconexSDK* sdk = tokenize_direct_setup(mockres);
    voxgig_value* params = v_map();
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("healthcheck/tokenize"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefinshieldconex_tokenize_direct");
}
