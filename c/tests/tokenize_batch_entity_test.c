...
int main(void) {
  ...
  Entity* e = bluefin_shieldconex_tokenize_batch(sdk, NULL);
  ...
  // the materialised items — either way every item is yielded.
  {
    ...
    Entity* se = bluefin_shieldconex_tokenize_batch(strsdk, NULL);
    PNError* serr = NULL;
    voxgig_value* items = tokenize_batch_stream(se, "list", NULL, NULL, &serr);
    ...
    Entity* pe = bluefin_shieldconex_tokenize_batch(plainsdk, NULL);
    PNError* perr = NULL;
    voxgig_value* pitems = tokenize_batch_stream(pe, "list", NULL, NULL, &perr);
