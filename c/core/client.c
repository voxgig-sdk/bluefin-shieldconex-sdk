// BluefinShieldconexSDK client (generated — mirrors the rust Main fragment).
...
BluefinShieldconexSDK* bluefinshieldconex_sdk_new(voxgig_value* options) {
  BluefinShieldconexSDK* sdk = (BluefinShieldconexSDK*)calloc(1, sizeof(BluefinShieldconexSDK));
...
static bool sdk_op_allowed(BluefinShieldconexSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(BluefinShieldconexSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  BluefinShieldconexSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(BluefinShieldconexSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "BluefinShieldconexSDK: graphql: %s", m);
...
BluefinShieldconexSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinShieldconexSDK* sdk = bluefinshieldconex_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
