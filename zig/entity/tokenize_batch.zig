// TokenizeBatch entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TokenizeBatchEntity = struct {
    name: []const u8 = "tokenize_batch",
    ...
    pub fn new(client: *sdk.BluefinShieldconexSDK, entopts_in: Value) *TokenizeBatchEntity {
        ...
        const e = h.A().create(TokenizeBatchEntity) catch unreachable;
        e.* = .{
            .name = "tokenize_batch",
            .client = client,
    ...
    fn ent_ctx(self: *TokenizeBatchEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TokenizeBatchEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *TokenizeBatchEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *TokenizeBatchEntity, ctx: *Context, post_done: *const fn (*TokenizeBatchEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *TokenizeBatchEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *TokenizeBatchEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *TokenizeBatchEntity, ctx: *Context, post_done: *const fn (*TokenizeBatchEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *TokenizeBatchEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TokenizeBatchEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TokenizeBatchEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TokenizeBatchEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TokenizeBatchEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TokenizeBatchEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *TokenizeBatchEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *TokenizeBatchEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *TokenizeBatchEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *TokenizeBatchEntity, ctx: *Context) void {
    ...
    pub fn update(self: *TokenizeBatchEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *TokenizeBatchEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
