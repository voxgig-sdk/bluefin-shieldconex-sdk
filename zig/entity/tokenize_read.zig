// TokenizeRead entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TokenizeReadEntity = struct {
    name: []const u8 = "tokenize_read",
    ...
    pub fn new(client: *sdk.BluefinShieldconexSDK, entopts_in: Value) *TokenizeReadEntity {
        ...
        const e = h.A().create(TokenizeReadEntity) catch unreachable;
        e.* = .{
            .name = "tokenize_read",
            .client = client,
    ...
    fn ent_ctx(self: *TokenizeReadEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TokenizeReadEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *TokenizeReadEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *TokenizeReadEntity, ctx: *Context, post_done: *const fn (*TokenizeReadEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *TokenizeReadEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *TokenizeReadEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *TokenizeReadEntity, ctx: *Context, post_done: *const fn (*TokenizeReadEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *TokenizeReadEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TokenizeReadEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TokenizeReadEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TokenizeReadEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TokenizeReadEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TokenizeReadEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *TokenizeReadEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *TokenizeReadEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *TokenizeReadEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *TokenizeReadEntity, ctx: *Context) void {
    ...
    pub fn update(self: *TokenizeReadEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *TokenizeReadEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
