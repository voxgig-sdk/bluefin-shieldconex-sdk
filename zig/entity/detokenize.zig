// Detokenize entity client (generated — mirrors the go/rust Entity fragment).
...
pub const EntResult = union(enum) {
    ok: *DetokenizeEntity,
    err: *errmod.BluefinShieldconexError,
};
...
pub const DetokenizeEntity = struct {
    name: []const u8 = "detokenize",
    client: *sdk.BluefinShieldconexSDK,
    utility: *Utility,
    entopts: Value,
    data: Value,
    mtch: Value,
    ...
    pub fn new(client: *sdk.BluefinShieldconexSDK, entopts_in: Value) *DetokenizeEntity {
        ...
        const e = h.A().create(DetokenizeEntity) catch unreachable;
        e.* = .{
            .name = "detokenize",
            .client = client,
    ...
    fn ent_ctx(self: *DetokenizeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *DetokenizeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *DetokenizeEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *DetokenizeEntity, ctx: *Context, post_done: *const fn (*DetokenizeEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *DetokenizeEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *DetokenizeEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *DetokenizeEntity, ctx: *Context, post_done: *const fn (*DetokenizeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *DetokenizeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *DetokenizeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return DetokenizeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *DetokenizeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *DetokenizeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *DetokenizeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *DetokenizeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *DetokenizeEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*DetokenizeEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = DetokenizeEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*DetokenizeEntity{} };
    }
    
    fn list_post_done(self: *DetokenizeEntity, ctx: *Context) void {
    ...
    pub fn create(self: *DetokenizeEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *DetokenizeEntity, ctx: *Context) void {
        if (ctx.result) |result| {
    ...
    pub fn update(self: *DetokenizeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *DetokenizeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
