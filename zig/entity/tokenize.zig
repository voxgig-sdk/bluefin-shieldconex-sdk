// Tokenize entity client (generated — mirrors the go/rust Entity fragment).
...
pub const TokenizeEntity = struct {
    name: []const u8 = "tokenize",
    ...
    pub fn new(client: *sdk.BluefinShieldconexSDK, entopts_in: Value) *TokenizeEntity {
        ...
        const e = h.A().create(TokenizeEntity) catch unreachable;
        e.* = .{
            .name = "tokenize",
            .client = client,
    ...
    fn ent_ctx(self: *TokenizeEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *TokenizeEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *TokenizeEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *TokenizeEntity, ctx: *Context, post_done: *const fn (*TokenizeEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *TokenizeEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *TokenizeEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *TokenizeEntity, ctx: *Context, post_done: *const fn (*TokenizeEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *TokenizeEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *TokenizeEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return TokenizeEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *TokenizeEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *TokenizeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *TokenizeEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *TokenizeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn list(self: *TokenizeEntity, reqmatch: Value, ctrl: Value) EntListResult {
        ...
        var items: std.ArrayList(*TokenizeEntity) = .empty;
        if (out == .array) {
            for (out.array.data.items) |entry| {
                const ent = TokenizeEntity.new(self.client, h.clone(self.entopts));
                if (entry == .object) {
        ...
        return EntListResult{ .ok = items.toOwnedSlice(h.A()) catch &[_]*TokenizeEntity{} };
    }
    
    fn list_post_done(self: *TokenizeEntity, ctx: *Context) void {
    ...
    pub fn create(self: *TokenizeEntity, reqdata: Value, ctrl: Value) EntResult {
        const ctx = self.utility.make_context(CtxSpec{
            .opname = "create",
            .ctrl = ctrl,
            .mtch = self.mtch,
    ...
    fn create_post_done(self: *TokenizeEntity, ctx: *Context) void {
        if (ctx.result) |result| {
    ...
    pub fn update(self: *TokenizeEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *TokenizeEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
