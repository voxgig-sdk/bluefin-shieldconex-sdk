// BluefinShieldconexSDK client (generated — mirrors the go/rust Main fragment).
...
pub const BluefinShieldconexSDK = struct {
    mode: []const u8 = "live",
    options: Value = .{ .null = {} },
    ...
    pub fn new(options: Value) *BluefinShieldconexSDK {
    ...
    pub fn new_with(options: Value, extend: []const Feature) *BluefinShieldconexSDK {
        const sdk = h.A().create(BluefinShieldconexSDK) catch unreachable;
        sdk.* = .{
            .mode = "live",
            .options = h.vnull(),
    ...
    pub fn options_map(self: *BluefinShieldconexSDK) Value {
        return h.clone(self.options);
    }

    pub fn get_utility(self: *BluefinShieldconexSDK) *Utility {
        return Utility.copy(self.sdkUtility);
    }

    pub fn get_root_ctx(self: *BluefinShieldconexSDK) *Context {
        return self.rootctx orelse unreachable;
    }

    pub fn prepare(self: *BluefinShieldconexSDK, fetchargs_in: Value) E!Value {
    ...
    pub fn direct(self: *BluefinShieldconexSDK, fetchargs_in: Value) Value {
    ...
    fn op_allowed(self: *BluefinShieldconexSDK, op: []const u8) bool {
    ...
    fn op_denied(self: *BluefinShieldconexSDK, op: []const u8) Value {
        const allow: []const u8 = switch (h.getpath(&.{ "allow", "op" }, self.options)) {
            .string => |s| s,
            else => "",
        };
        const msg = std.fmt.allocPrint(h.A(),
            "BluefinShieldconexSDK: {s}: operation not allowed by" ++
    ...
    // gate by passing it.
    fn raw_request(self: *BluefinShieldconexSDK, fetchargs_in: Value) Value {
    ...
    pub fn graphql(
        self: *BluefinShieldconexSDK, query: []const u8, variables: Value, ctrl: Value,
    ) Value {
        ...
        if (errors == .array and 0 < errors.array.data.items.len) {
            ...
            const msg = std.fmt.allocPrint(h.A(),
                "BluefinShieldconexSDK: graphql: {s}", .{m}) catch "";
    ...
    pub fn detokenize(self: *@This(), entopts: Value) *@import("../entity/detokenize.zig").DetokenizeEntity {
        return @import("../entity/detokenize.zig").DetokenizeEntity.new(self, entopts);
    }

    /// Tokenize entity bound to this client.
    pub fn tokenize(self: *@This(), entopts: Value) *@import("../entity/tokenize.zig").TokenizeEntity {
        return @import("../entity/tokenize.zig").TokenizeEntity.new(self, entopts);
    }

    /// TokenizeBatch entity bound to this client.
    pub fn tokenize_batch(self: *@This(), entopts: Value) *@import("../entity/tokenize_batch.zig").TokenizeBatchEntity {
        return @import("../entity/tokenize_batch.zig").TokenizeBatchEntity.new(self, entopts);
    }

    /// TokenizeRead entity bound to this client.
    pub fn tokenize_read(self: *@This(), entopts: Value) *@import("../entity/tokenize_read.zig").TokenizeReadEntity {
        return @import("../entity/tokenize_read.zig").TokenizeReadEntity.new(self, entopts);
    }

    /// Validate entity bound to this client.
    pub fn validate(self: *@This(), entopts: Value) *@import("../entity/validate.zig").ValidateEntity {
        return @import("../entity/validate.zig").ValidateEntity.new(self, entopts);
    }

};

pub fn test_sdk(testopts_in: Value, sdkopts_in: Value) *BluefinShieldconexSDK {
    ...
    const sdk = BluefinShieldconexSDK.new(sdkopts);
    sdk.mode = "test";
