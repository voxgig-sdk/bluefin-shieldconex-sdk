...
public class PipelineTest
{
    ...
    private static (BluefinShieldconexSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinShieldconexSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinShieldconexSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinShieldconexError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private class PlEntity : IEntity
    {
        ...
        // BluefinShieldconexEntityBase: `remove` marks the instance, which keeps the
    ...
    private static Utility UtilWith(BluefinShieldconexSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinShieldconexError pe && pe.Code == "boom",
