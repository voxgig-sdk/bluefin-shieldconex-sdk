...
class PrimaryUtilityTest {
  ...
  companion object {
    ...
    private var CLIENT: BluefinShieldconexSDK? = null
    ...
    fun run(): OmniResolver.Run {
      var r = RUN
      if (null == r) {
        val client = BluefinShieldconexSDK.testSDK()
        CLIENT = client
    ...
    fun client(): BluefinShieldconexSDK {
      run()
      return CLIENT!!
    }
  ...
  fun featureHookBasic() {
    val hookClient = BluefinShieldconexSDK.testSDK()
  ...
  fun featureInitBasic() {
    val initClient = BluefinShieldconexSDK.testSDK()
