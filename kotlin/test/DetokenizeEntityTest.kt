...
class DetokenizeEntityTest {
  ...
  @Test
  fun basic() {
    val setup = detokenizeBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create", "list")) {
      val reason = RunnerSupport.skipReason("entityOp", "detokenize.$op", mode)
    ...
    var detokenizeRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
  ...
  fun stream() {
    ...
    val setup = detokenizeBasicSetup(streamingActive)
    Assumptions.assumeFalse(
      setup.live,
      "stream test streams the seeded fixture data (unit mode only)",
    )

    val ent = setup.client.detokenize(null)
    ...
    // items.
    val setup2 = detokenizeBasicSetup(null)
    val ent2 = setup2.client.detokenize(null)
  ...
  companion object {
    fun detokenizeBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read detokenize test data: " + e.message, e)
      }
