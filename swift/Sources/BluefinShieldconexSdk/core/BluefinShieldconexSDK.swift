// BluefinShieldconex SDK client.
//
// SDK TYPES ARE MODULE-QUALIFIED IN THIS FILE (`BluefinShieldconexSdk.VMap`, not
...
// module - the generated module is <Name>Sdk, so `BluefinShieldconexSdk.` lands as
...
public final class BluefinShieldconexSDK {
  public var mode = "live"
  private var options: BluefinShieldconexSdk.VMap = BluefinShieldconexSdk.VMap()
  private let utility: BluefinShieldconexSdk.Utility
  public var features: [BaseFeature] = []
  private var rootctx: BluefinShieldconexSdk.Context!

  public init(_ optionsIn: BluefinShieldconexSdk.VMap? = nil) {
    utility = BluefinShieldconexSdk.Utility()
    ...
    var ctxmap: [String: Any?] = [
      ...
      "utility": utility,
      "config": config,
      "shared": BluefinShieldconexSdk.VMap(),
    ]
    ...
    let featureOpts = gp(options, "feature").asMap ?? BluefinShieldconexSdk.VMap()
  ...
  public func optionsMap() -> BluefinShieldconexSdk.VMap {
    return clone(.map(options)).asMap ?? BluefinShieldconexSdk.VMap()
  }

  public func getUtility() -> BluefinShieldconexSdk.Utility {
    return BluefinShieldconexSdk.Utility.copy(utility)
  }

  public func getRootCtx() -> BluefinShieldconexSdk.Context {
    return rootctx
  }

  public func prepare(_ fetchargsIn: BluefinShieldconexSdk.VMap?) throws -> BluefinShieldconexSdk.VMap {
    let utility = self.utility

    let fetchargs = fetchargsIn ?? BluefinShieldconexSdk.VMap()

    let ctrl = gp(fetchargs, "ctrl").asMap ?? BluefinShieldconexSdk.VMap()
    ...
    let pathParams = gp(fetchargs, "params").asMap ?? BluefinShieldconexSdk.VMap()
    let query = gp(fetchargs, "query").asMap ?? BluefinShieldconexSdk.VMap()
    ...
    let specmap = BluefinShieldconexSdk.VMap()
    ...
    ctx.spec = BluefinShieldconexSdk.Spec(specmap)
  ...
  public func direct(_ fetchargsIn: BluefinShieldconexSdk.VMap?) -> BluefinShieldconexSdk.VMap {
    if !opAllowed("direct") {
  ...
  private func opDenied(_ op: String) -> BluefinShieldconexSdk.VMap {
    let allow = gpath(options, "allow", "op").asString ?? ""
    let r = BluefinShieldconexSdk.VMap()
    r.entries["ok"] = .bool(false)
    r.entries["err"] = .nat(BluefinShieldconexError(
      op + "_allow",
      "BluefinShieldconexSDK: \(op): operation not allowed by SDK option "
  ...
  // gate by passing it.
  private func rawRequest(_ fetchargsIn: BluefinShieldconexSdk.VMap?) -> BluefinShieldconexSdk.VMap {
    let utility = self.utility

    let fetchdef: BluefinShieldconexSdk.VMap
    do {
    ...
    let fetched: BluefinShieldconexSdk.Value
    do {
    ...
    if let fm = fetched.asMap {
      ...
      var jsonData: BluefinShieldconexSdk.Value = .noval
      if !noBody, let jf = gp(fm, "json").asNative as? BluefinShieldconexSdk.NativeCall0 {
        jsonData = jf()
      }
  ...
  public func graphql(
    _ query: String, _ variables: BluefinShieldconexSdk.VMap? = nil, _ ctrl: BluefinShieldconexSdk.VMap? = nil
  ) -> BluefinShieldconexSdk.VMap {
    if !opAllowed("graphql") {
      return opDenied("graphql")
    }

    let headers = BluefinShieldconexSdk.VMap()
    headers.entries["content-type"] = .string("application/json")

    let body = BluefinShieldconexSdk.VMap()
    body.entries["query"] = .string(query)
    body.entries["variables"] = .map(variables ?? BluefinShieldconexSdk.VMap())

    let fetchargs = BluefinShieldconexSdk.VMap()
    fetchargs.entries["method"] = .string("POST")
    fetchargs.entries["headers"] = .map(headers)
    fetchargs.entries["body"] = .map(body)
    fetchargs.entries["ctrl"] = .map(ctrl ?? BluefinShieldconexSdk.VMap())
    ...
    res.entries["err"] = .nat(BluefinShieldconexError(
      "graphql_error", "BluefinShieldconexSDK: graphql: " + msg, nil))
  ...
  // Detokenize returns a Detokenize entity bound to this client.
  // Idiomatic usage: try client.Detokenize().list(nil) or
  // try client.Detokenize().load(vm(("id", .string("..."))), nil).
  public func Detokenize(_ entopts: VMap? = nil) -> BluefinShieldconexEntityBase {
    return DetokenizeEntity(self, entopts)
  }

  // Tokenize returns a Tokenize entity bound to this client.
  // Idiomatic usage: try client.Tokenize().list(nil) or
  // try client.Tokenize().load(vm(("id", .string("..."))), nil).
  public func Tokenize(_ entopts: VMap? = nil) -> BluefinShieldconexEntityBase {
    return TokenizeEntity(self, entopts)
  }

  // TokenizeBatch returns a TokenizeBatch entity bound to this client.
  // Idiomatic usage: try client.TokenizeBatch().list(nil) or
  // try client.TokenizeBatch().load(vm(("id", .string("..."))), nil).
  public func TokenizeBatch(_ entopts: VMap? = nil) -> BluefinShieldconexEntityBase {
    return TokenizeBatchEntity(self, entopts)
  }

  // TokenizeRead returns a TokenizeRead entity bound to this client.
  // Idiomatic usage: try client.TokenizeRead().list(nil) or
  // try client.TokenizeRead().load(vm(("id", .string("..."))), nil).
  public func TokenizeRead(_ entopts: VMap? = nil) -> BluefinShieldconexEntityBase {
    return TokenizeReadEntity(self, entopts)
  }

  // Validate returns a Validate entity bound to this client.
  // Idiomatic usage: try client.Validate().list(nil) or
  // try client.Validate().load(vm(("id", .string("..."))), nil).
  public func Validate(_ entopts: VMap? = nil) -> BluefinShieldconexEntityBase {
    return ValidateEntity(self, entopts)
  }


  public static func testSDK(_ testoptsIn: BluefinShieldconexSdk.VMap?, _ sdkoptsIn: BluefinShieldconexSdk.VMap?) -> BluefinShieldconexSDK {
    let sdkopts = clone(.map(sdkoptsIn ?? BluefinShieldconexSdk.VMap())).asMap ?? BluefinShieldconexSdk.VMap()

    let testopts = clone(.map(testoptsIn ?? BluefinShieldconexSdk.VMap())).asMap ?? BluefinShieldconexSdk.VMap()
    testopts.entries["active"] = .bool(true)

    _ = setpath(.map(sdkopts), jtp("feature", "test"), .map(testopts))

    let sdk = BluefinShieldconexSDK(sdkopts)
    sdk.mode = "test"
    return sdk
  }
}
