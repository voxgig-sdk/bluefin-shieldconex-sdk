...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: BluefinShieldconexSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: BluefinShieldconexSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> BluefinShieldconexSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: BluefinShieldconexSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.client = { options in
      ...
      return sdkProvider(BluefinShieldconexSDK.testSDK(nil, opts), true)
    }

    // Client options may reference the runner store.
    provider.inject = { options, store in
      let o = tovalue(options)
      _ = BluefinShieldconexSdk.inject(o, tovalue(store))
      return tojson(o)
    }
    ...
    provider.errify = { err in
      if let sdkerr = err as? BluefinShieldconexError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("BluefinShieldconexError")),
  ...
  final class Run {
    ...
    let client: BluefinShieldconexSDK

    private let pack: Omni.RunPack

    fileprivate init(_ pack: Omni.RunPack, _ client: BluefinShieldconexSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: BluefinShieldconexSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: BluefinShieldconexSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: BluefinShieldconexSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: BluefinShieldconexSDK, _ utility: Utility)
    -> Context
  {
