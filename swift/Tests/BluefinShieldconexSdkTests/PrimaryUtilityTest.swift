...
final class PrimaryUtilityTest: XCTestCase {
  ...
  private static func primaryRun() throws -> OmniResolver.Run {
    ...
    let run = try OmniResolver.makeRunner(
      SdkRunner.testJsonPath(), BluefinShieldconexSDK.testSDK(nil, nil))("primary")
    RUN = run
    return run
  }

  // XCTestCase has no custom init; set these up per test.
  private var client: BluefinShieldconexSDK!
  private var utility: Utility!

  override func setUp() {
    super.setUp()
    client = BluefinShieldconexSDK.testSDK(nil, nil)
  ...
  private func makeTestCtx(_ client: BluefinShieldconexSDK, _ utility: Utility,
  ...
  private func makeTestFullCtx(_ client: BluefinShieldconexSDK, _ utility: Utility) -> Context {
  ...
  func testFeatureHookBasic() {
    let hookClient = BluefinShieldconexSDK.testSDK(nil, nil)
  ...
  func testFeatureInitBasic() {
    let initClient = BluefinShieldconexSDK.testSDK(nil, nil)
  ...
  func testExtendOptionInstallsAndInitialisesTheFeature() {
    ...
    let client = BluefinShieldconexSDK.testSDK(nil, opts)
  ...
  func testFetcherLive() throws {
    ...
    let liveClient = BluefinShieldconexSDK(opts)
  ...
  func testFetcherBlockedTestMode() {
    ...
    let opts = VMap()
    opts.entries["system"] = .map(vm(("fetch", .nat(fetch))))
    let blockedClient = BluefinShieldconexSDK(opts)
  ...
  func testMakeSpecBasic() throws {
    ...
    let specClient = BluefinShieldconexSDK.testSDK(nil, setupOpts)
  ...
  func testPrepareAuthBasic() throws {
    ...
    let authClient = BluefinShieldconexSDK.testSDK(nil, setupOpts)
