...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinShieldconexSDK, Utility) {
    let client = BluefinShieldconexSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinShieldconexSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinShieldconexError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinShieldconexSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinShieldconexError)?.code, "boom")
  }
