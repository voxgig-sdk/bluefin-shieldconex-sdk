# bluefin-shieldconex-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BluefinShieldconex SDK as
two agent tools — `bluefin-shieldconex_list` and `bluefin-shieldconex_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bluefin-shieldconex-mcp)
make build

# 2. Provide credentials via the environment
export BLUEFIN_SHIELDCONEX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bluefin-shieldconex \
  -- /absolute/path/to/bluefin-shieldconex-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bluefin-shieldconex-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bluefin-shieldconex_list: first page of records
{ "entity": "detokenize" }
{ "entity": "detokenize", "query": { } }

// bluefin-shieldconex_load: one record by id
{ "entity": "detokenize", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bluefin-shieldconex-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bluefin-shieldconex \
     -- "$PWD"/dist/*/bluefin-shieldconex-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bluefin-shieldconex_list` and `bluefin-shieldconex_load` tools now appear
   in new sessions. Ask the agent to *"list detokenize using bluefin-shieldconex"*
   and it calls `bluefin-shieldconex_list` with `{"entity":"detokenize"}`.
...
```sh
export BLUEFIN_SHIELDCONEX_APIKEY=sk_live_xxx            # API key
export BLUEFIN_SHIELDCONEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bluefin-shieldconex_list` tool
...
### Call the `bluefin-shieldconex_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bluefin-shieldconex_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bluefin-shieldconex_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BLUEFIN_SHIELDCONEX_APIKEY` | API key sent with every request. |
| `BLUEFIN_SHIELDCONEX_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

detokenize | tokenize | tokenize_batch | tokenize_read | validate

### Smoke test via HTTP (raw JSON-RPC)

```sh
./bluefin-shieldconex-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bluefin-shieldconex_load","arguments":{"entity":"detokenize","query":{"id":1}}}}'
```
